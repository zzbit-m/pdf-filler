...
class TestSuggestionsRotation:
    def _make_pdf_with_labels(self, rotation: int) -> tuple[str, int, float, float]:
        doc = fitz.open()
        page = doc.new_page(width=400, height=500)
        page.insert_text((80, 400), "Name", fontsize=12)
        page.set_rotation(rotation)
        media_w = page.mediabox_size.x
        media_h = page.mediabox_size.y
        pdf_bytes = doc.write()
        ...
        assert resp.status_code == 200, resp.text
        return resp.json()["pdf_id"], rotation, media_w, media_h

    def test_suggestions_rot0_no_conversion(self):
        pdf_id, rot, mw, mh = self._make_pdf_with_labels(0)
        resp = client.get(f"/preview/suggest/{pdf_id}/1?columns=Name")
    ...
    def test_suggestions_rot90_visual_coords(self):
        pdf_id, rot, mw, mh = self._make_pdf_with_labels(90)
        resp = client.get(f"/preview/suggest/{pdf_id}/1?columns=Name")
        assert resp.status_code == 200
        suggestions = resp.json()["suggestions"]
        assert len(suggestions) == 1
    ...
    def test_suggestions_rot270_visual_coords(self):
        pdf_id, rot, mw, mh = self._make_pdf_with_labels(270)
        resp = client.get(f"/preview/suggest/{pdf_id}/1?columns=Name")
        assert resp.status_code == 200
        suggestions = resp.json()["suggestions"]
        assert len(suggestions) == 1
